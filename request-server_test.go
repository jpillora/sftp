...
func TestRequestCache(t *testing.T) {
	...
	fooReader, fooWriter, fooReadWriter := foo.getAllReaderWriters()
	copyReader, copyWriter, copyReadWriter := _foo.getAllReaderWriters()
	assert.Equal(t, fooReader, copyReader)
	assert.Equal(t, fooWriter, copyWriter)
	assert.Equal(t, fooReadWriter, copyReadWriter)
	assert.Equal(t, foo.getListerAt(), _foo.getListerAt())
	assert.Equal(t, foo.lsNext(), _foo.lsNext())
